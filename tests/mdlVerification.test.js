...
const getSessionTranscriptBytes = (oid4vpData, mdocGeneratedNonce) =>
  encodeCbor([
    "OIDC4VPHandover",
...
async function verifyMdlVpToken(vpToken, sessionData, trustedCerts) {
  ...
  console.log(
    "Token buffer first 50 bytes:",
    encodedDeviceResponse.subarray(0, 50).toString("hex")
  );
  ...
  try {
    ...
    holderNonce =
      diagnosticInfo?.deviceResponse?.handover?.sessionTranscript?.[0];
  } catch (diagError) {
    console.warn(
      "Could not extract diagnostic information:",
      diagError.message
    );
  ...
  try {
    ...
    console.warn(
      "Error extracting claims from namespace:",
      claimsError.message
    );
    // Get available namespaces for debugging
    const availableNamespaces = mdoc.getIssuerNameSpaces
      ? mdoc.getIssuerNameSpaces()
      : "getIssuerNameSpaces not available";
...
describe("mDL VP Token Verification", () => {
  ...
  const trustedCerts = [
    fs.readFileSync("./x509EC/client_certificate.crt", "utf8"),
  ];
  ...
  describe("VP Token Processing", () => {
    ...
    it.skip("should validate mDL token CBOR structure", async () => {
      ...
      try {
        ...
        console.log(
          `First 20 bytes (hex): ${buffer.subarray(0, 20).toString("hex")}`
        );
        ...
        console.log(
          `Has version: ${hasVersion} ${
            hasVersion ? `(${decoded.version})` : ""
          }`
        );
        console.log(
          `Has documents: ${hasDocuments} ${
            hasDocuments ? `(${decoded.documents?.length} docs)` : ""
          }`
        );
        console.log(
          `Has status: ${hasStatus} ${hasStatus ? `(${decoded.status})` : ""}`
        );
        ...
        if (hasDocuments && decoded.documents?.length > 0) {
          ...
          if (doc.issuerSigned) {
            ...
            if (doc.issuerSigned.nameSpaces) {
              console.log(
                `Available namespaces: ${Object.keys(
              ...
              const isoNamespace =
                doc.issuerSigned.nameSpaces["org.iso.18013.5.1"];
              if (isoNamespace) {
                console.log(
                  `ISO namespace has ${isoNamespace.length} elements`
                );
                ...
                isoNamespace.slice(0, 5).forEach((element, index) => {
                  try {
                    ...
                    console.log(
                      `Element ${index}: Could not decode - ${e.message}`
                    );
                    console.log(
                      `Element ${index} type:`,
                      typeof element,
        ...
        assert(
          decoded.documents?.length > 0,
          "mDL should have at least one document"
        );
    ...
    it.skip("should demonstrate the @auth0/mdl library bug", async () => {
      ...
      try {
        ...
        assert(
          error.message.includes("namespace.entries is not a function"),
          "Should fail with the namespace.entries bug"
        );

        console.log(
          "✅ Bug confirmed: The library expects Map objects but receives plain objects"
        );
        console.log(
          "✅ This confirms our token is valid but the library has a compatibility issue"
        );
      }
    });
    ...
    it.skip("should successfully verify the mDL token with @animo-id/mdoc", async () => {
      ...
      try {
        ...
        const mdoc = await verifier.verify({
          ...
          trustedCertificates: [
            new Uint8Array(fs.readFileSync("./x509EC/client_certificate.crt")),
          ],
        });
        ...
        const claims =
          mdoc.documents[0].getIssuerNameSpace("org.iso.18013.5.1");
        console.log("Extracted claims:", claims);

        assert.ok(
          mdoc,
          "mdoc object should be returned on successful verification."
        );
        assert.ok(claims, "Should be able to extract claims.");

        const givenNameClaim = Object.values(claims).find(
          (c) => c.elementIdentifier === "given_name"
        );
        assert.ok(givenNameClaim, "given_name claim should exist");
        assert.strictEqual(
          givenNameClaim.elementValue,
          "Hanna",
          "The given_name should be Hanna."
        );
      } catch (error) {
        console.error("❌ Animo verification failed:", error);
        console.error(error.stack);
        assert.fail(
          `Verification with @animo-id/mdoc failed: ${error.message}`
        );
      }
    });
    ...
    it.skip("should create a mock unit test for mDL verification logic", async () => {
      ...
      console.log(
        "Testing the logical flow that should work once the library is fixed"
      );
      ...
      assert(
        isoClaims.given_name === "Hanna",
        "Should have correct given name"
      );
      assert(
        isoClaims.surname === "Matkalainen",
        "Should have correct surname"
      );
      assert(
        isoClaims.id === "777e431d-3ed3-4e04-9c63-8fe29b615f3b",
    ...
    it.skip("should confirm diagnostic extraction also fails due to library bug", async () => {
      ...
      try {
        ...
        console.log(
          "❌ Diagnostic extraction failed as expected:",
          diagError.message
        );

        // Verify this is the same bug
        assert(
          diagError.message.includes("namespace.entries is not a function"),
          "Should fail with the same namespace.entries bug"
        );

        console.log(
          "✅ Confirmed: Both verify() and getDiagnosticInformation() hit the same bug"
        );
      }
    });
    ...
    it.skip("should process the provided mDL VP token payload", async () => {
      ...
      try {
        const result = await verifyMdlVpToken(
          testPayload,
          testSessionData,
          trustedCerts
        );
        ...
        assert(
          result.hasOwnProperty("claims"),
          "Result should have claims property"
        );
        assert(
          result.hasOwnProperty("holderNonce"),
          "Result should have holderNonce property"
        );
        assert(
          result.hasOwnProperty("sessionTranscript"),
          "Result should have sessionTranscript property"
        );

        console.log(
          "Successfully processed VP token with claims:",
          result.claims
        );
      } catch (error) {
    ...
    it.skip("should successfully verify the mDL token with @m-doc/mdl", async () => {
      ...
      try {
        ...
        const decoded = mdoc.MDoc?.fromBuffer
          ? mdoc.MDoc.fromBuffer(arrayBuffer)
          : null;
        ...
        if (decoded) {
          ...
          console.log(
            "Available methods:",
            Object.getOwnPropertyNames(Object.getPrototypeOf(decoded))
          );
          ...
          console.log(
            "Available methods for MDoc:",
            mdoc.MDoc ? Object.getOwnPropertyNames(mdoc.MDoc) : "MDoc not found"
          );
        }
      } catch (error) {
    ...
    it.skip("should fail to parse the mDL token with @m-doc/mdl", async () => {
      ...
      try {
        ...
        assert.fail(
          "The @m-doc/mdl library did not throw an error as expected."
        );
      } catch (error) {
        ...
        assert.strictEqual(
          ...
          "Should fail with the expected ArrayBuffer error."
        );
    ...
    it.skip("should successfully parse the mDL token with @protokoll/mdoc-client", async () => {
      ...
      try {
        ...
        console.log(
          "Available exports from @protokoll/mdoc-client:",
          Object.keys(mdocClient)
        );
        ...
        if (mdocClient.Verifier) {
          ...
          if (mdocClient.default) {
            console.log(
              ...
              Object.keys(mdocClient.default)
            );
          }
        }
      } catch (error) {
    ...
    it.skip("should explore @protokoll/mdoc-client API and provide alternatives summary", async () => {
      ...
      console.log(
        "   - Still has the 'namespace.entries is not a function' bug"
      );
      console.log(
        "   - Unable to parse CBOR correctly due to internal parser issues"
      );
      console.log("");
      ...
      console.log(
        "   - Promising API but fails on Buffer/ArrayBuffer incompatibility"
      );
      console.log("");
      ...
      console.log(
        "2. ✅ Use OpenWallet Foundation identity-credential (not on npm but available on GitHub)"
      );
      console.log("3. ✅ Consider Procivis One Core for production use");
      console.log(
        "4. ✅ Wait for @auth0/mdl bug fixes or contribute fix ourselves"
      );
      console.log("");
      console.log(
        "For immediate testing, our cbor-x decoder proves the token is valid."
      );
      console.log(
        "For production, consider the OpenWallet Foundation or Procivis solutions."
      );
    });
    ...
    it.skip("should create a reusable mDL verification function", async () => {
            ...
            if (isoNamespace && Array.isArray(isoNamespace)) {
              isoNamespace.forEach((element) => {
                try {
                  ...
                  if (
                    elementDecoded?.elementIdentifier &&
                    elementDecoded.elementValue !== undefined
                  ) {
                    claims[elementDecoded.elementIdentifier] =
                      elementDecoded.elementValue;
                  }
                } catch (e) {
      ...
      assert.strictEqual(
        Object.keys(filteredResult.claims).length,
        3,
      ...
      assert.ok(
        filteredResult.claims.email_address,
        "Should include email_address"
      );
      assert.ok(
        !filteredResult.claims.phone,
    ...
    it.skip("should integrate custom verification with verifierRoutes logic", async () => {
      ...
      const { verifyMdlToken, validateMdlClaims } = await import(
        "../utils/mdlVerification.js"
      );
      ...
      try {
        ...
        console.log(
          `  📊 Requested fields: ${sessionData.sdsRequested.length}`
        );
        ...
        assert.strictEqual(
          ...
          3,
          "Should return exactly 3 requested fields"
        );
        ...
        assert.ok(
          !claims.phone,
          "Should NOT include non-requested phone field"
        );
    ...
    it.skip("should build custom mDL verification using cbor-x decoder", async () => {
      ...
      console.log(
        "Building reliable verification logic using cbor-x as foundation"
      );
      ...
      try {
        ...
        if (
          ...
          throw new Error(
            "Invalid mDL structure: missing version or documents"
          );
        }
        ...
        if (document.issuerSigned && document.issuerSigned.nameSpaces) {
          ...
          if (isoNamespace && Array.isArray(isoNamespace)) {
            console.log(
              `🔍 Processing ${isoNamespace.length} elements from ISO namespace`
            );
              ...
              try {
                ...
                if (
                  ...
                  claims[elementDecoded.elementIdentifier] =
                    elementDecoded.elementValue;
                  console.log(
                    `  ✓ ${elementDecoded.elementIdentifier}: ${elementDecoded.elementValue}`
                  );
                }
              } catch (e) {
                console.warn(
                  `  ⚠️  Could not decode element ${index}: ${e.message}`
                );
              }
            });
          }
        ...
        const requiredFields = getSDsFromPresentationDef(
        ...
        assert.strictEqual(
          claims.given_name,
          "Hanna",
          "Should extract correct given name"
        );
        assert.strictEqual(
          claims.surname,
          "Matkalainen",
          "Should extract correct surname"
        );
    ...
    it.skip("should build custom mDL verification using cbor-x decoder for second payload", async () => {
      ...
      try {
        ...
        console.log(
          `📄 Number of documents: ${deviceResponse.documents.length}`
        );
        ...
        if (!validationResult) {
          console.warn(
            `⚠️  Validation failed: claims do not match requested fields`
          );
        } else {
    ...
    it.skip("should verify mDL token using verifyMdlToken function with selective disclosure", async () => {
